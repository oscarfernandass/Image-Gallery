...
const fetchFlickrData = async () => {
  ...
  useEffect(() => {
    if (photos.length === 20 && loadedPhotosCount === 20) {
      setLoading(false); // All 20 photos are loaded, set loading to false
    }
  }, [photos, loadedPhotosCount]);
    ...
    <View style={styles.container}>
      ...
      (
        <FlatList
          ...
          keyExtractor={(item) => item.id}
          renderItem={({ item }) => <Post title={item.title} imageUrl={item.url_s} 
          onLoad={() => setLoadedPhotosCount((prevCount) => prevCount + 1)}
          />}
          ItemSeparatorComponent={() => <View style={{ borderBottomWidth: 5, borderBottomColor: 'lightgreen' }} />}
          refreshControl={
            <RefreshControl refreshing={refreshing} onRefresh={onRefresh} />
